...
parser.add_argument('--mod', default='lite', type=str, help='the version of QRes network, can be "full" (3.00k) or "lite" (1.37k).')
...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, t, u, v, layers):
        ...
        if args.lr_scheduler:
            ...
            decayed_lr = tf.compat.v1.train.exponential_decay(learning_rate,
                                            global_step, 1000,
                                            0.98, staircase=True)
            self.optimizer_Adam = tf.compat.v1.train.AdamOptimizer(decayed_lr)
    ...
    def train(self, nIter, checkpoint=False): 

        tf_dict = {self.x_tf: self.x, self.y_tf: self.y, self.t_tf: self.t,
                   self.u_tf: self.u, self.v_tf: self.v}
        
        scipy.io.loadmat('../Data/cylinder_nektar_wake.mat')
    ...
    def checkpoint(self, epoch):
        ...
        np.save("saved/p_pred_qres_%s_%dk.npy" % (args.mod, int(epoch/1000)), p_pred)
...
if __name__ == "__main__": 
    ...
    if args.mod == "full":
        layers = [3, 14, 14, 14, 14, 14, 14, 14, 14, 2]
    else:
        layers = [3, 14, 14, 14, 14, 2]
    ...
    np.save("saved/p_pred_qres_%s_final.npy" % args.mod, p_pred)
    np.save("saved/p_true.npy", p_star)
    ...
    with open('results.txt', 'w') as f:
        ...
        lines.append('Error l1: %.5f%%\n' % (error_lambda_1))                             
        lines.append('Error l2: %.5f%%\n' % (error_lambda_2))  
    
        lines.append('Error l1: %.5f%%\n' % (error_lambda_1_noisy))                             
